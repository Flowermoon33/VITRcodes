...
ARcor <- function(p,rho) {
  exponent <- abs(matrix(1, nrow = p, ncol = p) - diag(p))
  return(rho^exponent)
}
#---------New cov structure-------
NEWCOV<-function(pvec,rovec,gamma){
  p<-sum(pvec)
  M<-matrix(gamma,nrow = p,ncol = p)
  d<-c(0,cumsum(pvec))
  for(i in 1:length(pvec)){
    M[(1+d[i]):d[i+1],(1+d[i]):d[i+1]]<-ARcor(pvec[i],rovec[i])
  }
  if(det(M)<0){print("Covariance is Singular")}
  return(M)
}
#========================================
...
#----------------------------------------------
EmpSize<-function(reppower,repempinull,n,pvec,rovec,gamma){
  p<-sum(pvec)
  SigmaMat0<-NEWCOV(pvec,rovec,gamma=0)
  SigmaMat<-NEWCOV(pvec,rovec,gamma)
  p<-sum(pvec)
  k<-length(pvec)
  c<-1
  AsPar<-Par(n,rep(c,k))
...
#----------------------------------------------
Alpha_AdjPower<-function(reppower,repempinull,n,pvec,rovec,gamma){
  SigmaMat0<-NEWCOV(pvec,rovec,gamma=0)
  SigmaMat<-NEWCOV(pvec,rovec,gamma)
  p<-sum(pvec)
  k<-length(pvec)
...
#@@@@@@@@@@@@@@ Table 12 @@@@@@@@@@@@@@@@@@@@@@@@@ 
#---------------gamma=0---------------------------
Sim1<-EmpSize( reppower=5000,repempinull=5000,n=180,pvec=c(40,50,60),rovec=c(0.75,0.85,0.95),gamma=0)
Sim2<-EmpSize( reppower=5000,repempinull=5000,n=200,pvec=c(250,300,350),rovec=c(0.75,0.85,0.95),gamma=0)
Sim3<-EmpSize( reppower=5000,repempinull=5000,n=250,pvec=c(500,550,600),rovec=c(0.75,0.85,0.95),gamma=0)
Sim4<-EmpSize( reppower=5000,repempinull=5000,n=300,pvec=c(1000,1100,1200),rovec=c(0.75,0.85,0.95),gamma=0)
#------------------------------------------------
#----------------gamma=0.1-----------------------
Sim5<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=180,pvec=c(40,50,60),rovec=c(0.75,0.85,0.95),gamma=0.1)
Sim6<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=200,pvec=c(250,300,350),rovec=c(0.75,0.85,0.95),gamma=0.1)
Sim7<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=250,pvec=c(500,550,600),rovec=c(0.75,0.85,0.95),gamma=0.1)
Sim8<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=300,pvec=c(1000,1100,1200),rovec=c(0.75,0.85,0.95),gamma=0.1)
#------------------------------------------------
#----------------gamma=0.2-----------------------
Sim9<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=180,pvec=c(40,50,60),rovec=c(0.75,0.85,0.95),gamma=0.2)
Sim10<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=200,pvec=c(250,300,350),rovec=c(0.75,0.85,0.95),gamma=0.2)
Sim11<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=250,pvec=c(500,550,600),rovec=c(0.75,0.85,0.95),gamma=0.2)
Sim12<-Alpha_AdjPower( reppower=5000,repempinull=5000,n=300,pvec=c(1000,1100,1200),rovec=c(0.75,0.85,0.95),gamma=0.2)
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
Table12<-rbind(Sim1,Sim2,Sim3,Sim4,Sim5,Sim6,Sim7,Sim8,Sim9,Sim10,Sim11,Sim12)
